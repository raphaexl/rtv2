...
/*   Updated: 2019/07/02 10:37:27 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_object		*ft_cylindre_new(void)
{
	...
	if ((new = (t_object *)malloc(sizeof(*new))))
	{
		...
		new->material = (t_material){DIFFUSE, {1.0, 0.0, 0.0, 0.0}, {ft_rand48(), ft_rand48(), ft_rand48()},
...
int				ft_cylindre_compute(t_object *p, t_intersect *in)
{
	...
	ret = ft_cylindre_intersect(&p->e.cylinder, &r, &in->t);
	if (!ret)
		return (0);
	in->current = p;
...
int				ft_cylindre_intersect(t_cylinder *c, t_ray *r, float *t)
{
	...
	if (m0 < -c->height)
	{
		...
		/*	float th = t0 + (t1 - t0) * (m0 + c->height) / (m0 - m1);
			if (th < 1e-6 || th > *t)
				return (0);
			*t = th;*/
			if (ft_cylindre_cap(r, t, ft_vec3_kmult(-c->height, c->v), ft_vec3_kmult(-1, c->v)))
				return (-1);
		}
	}
	...
	else if (m0 > c->height)
	{
		...
		/*	float th = t0 + (t1 - t0) * (m0 - c->height) / (m0 - m1);
			...
			*t = th;
			return (2);*/
			//return (ft_cylindre_cap(r, t, ft_vec3_kmult(c->height, c->v), ft_vec3_kmult(1.0, c->v)));
...
int			ft_cylindre_cap(t_ray *r, float *t, t_vec3 pos, t_vec3 n)
{
	float	ddn;
	float	t1;
	t_vec3	dist;
