...
/*   Updated: 2019/06/15 16:22:06 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_material	ft_material_read(int fd)
{
	...
	if (ft_get_next_line(fd, &line) != 1 && (!line))
		ft_print_error("material properties diffuse ? GLOSSY ? :)");
	if (!ft_strcmp(ft_strcapitalize(line), "DIFFUSE"))
		material.type = DIFFUSE;
	else if (!ft_strcmp(ft_strcapitalize(line), "SPECULAR"))
		material.type = SPECULAR;
	else if (!ft_strcmp(ft_strcapitalize(line), "METAL"))
		material.type = METAL;
	else if (!ft_strcmp(ft_strcapitalize(line), "DIELECTRIC"))
