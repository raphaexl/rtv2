...
/*   Updated: 2019/06/03 19:13:10 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_cone_compute(t_object *p, t_intersect *in)
{
	...
	else
	{
		...
		in->n = ft_vec3_sub(in->p, ft_vec3_kmult(m* (1 + 
						tan(p->e.cone.angle / 2.0) * tan(p->e.cone.angle / 2.0)),
				   	p->e.cone.v));
	}
...
int				ft_cone_intersect(t_cone *c, t_ray *r, float *t)
{
	...
	float m;
	m = ft_vec3_dot(r->dir, c->v) * (t0);
	m += ft_vec3_dot(ft_vec3_sum(x, ft_vec3_kmult(c->height, c->v)), c->v);
	if (m0 > 0 && m < 0)
	{
		return (ft_min_ray(t0, t1, t));
		if (t0 > 1e-6 && t0 < *t)
	...
	{
	m = ft_vec3_dot(r->dir, c->v) * (t1);
	m += ft_vec3_dot(ft_vec3_sub(x, ft_vec3_kmult(c->height, c->v)), c->v);
	if (m1 > 0 && m < 0)
	{
		return (ft_min_ray(t0, t1, t));
		if (t0 > 1e-6 && t0 < *t)
...
/*	else
	{
	 ...
	 if (ddn > 1e-6)
	 {
	  ...
	  if (t3 > 1e-6 && t3 < *t )//&& (ft_vec3_dot(ft_vec3_sub(ft_vec3_kmult(t3, r->dir), co)) <= radius * radius))
	  {
		  *t = t3;
		  printf("%.2f\t %.2f\t %.2f\n", t0, t1, t3);
		  return (2);
