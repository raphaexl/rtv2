...
/*   Updated: 2019/07/23 13:45:35 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_button	ft_button_new(t_ptr *ptr, Uint32 c, Uint32 c2, const char *m)
{
	...
	if (!(surf = SDL_CreateRGBSurface(0, DEFAULT_W1, DEFAULT_H, 32, RMASK, GMASK, BMASK, AMASK)))
	...
	w.tex = ft_get_render_text(ptr->renderer, ft_font(m, DEFAULT_FONT, DEFAULT_FSIZE),
			ft_sdl_color(c2));
...
t_button	*ft_button_create(t_ptr *ptr, Uint32 c, Uint32 c2, const char *m)
{
	...
	if (!(w->img = SDL_CreateTextureFromSurface(ptr->renderer, surf)))
		ft_print_error("Creating button 's Texture");
	if (!(w->tex = ft_get_render_text(ptr->renderer, ft_font(m, DEFAULT_FONT, DEFAULT_FSIZE),
					ft_sdl_color(c2))))
...
{
	if ((x < w->pos[0].x) || (x > w->pos[0].x + w->pos[0].w) || (y < w->pos[0].y) || (y > w->pos[0].y + w->pos[0].h))
		return (0);
	return (1);
}
