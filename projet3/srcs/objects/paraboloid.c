...
/*   Updated: 2019/09/17 10:55:12 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vec3		ft_normal_paraboloid(t_paraboloid *pa, t_vec3 p)
{
	...
	return (ft_vec3_normalized(ft_vec3_sub(p, ft_vec3_kmult(m + pa->k, pa->v))));
}
...
int			ft_paraboloid_intersect(t_paraboloid *p, t_ray *r, float *t)
{
	...
	d.b = 2.0 * (ft_vec3_dot(r->dir, x) - ft_vec3_dot(r->dir, p->v) * (ft_vec3_dot(x, p->v) + 2.0 * p->k));
	d.c = ft_vec3_dot(x, x) - ft_vec3_dot(x, p->v) * (ft_vec3_dot(x, p->v) + 4.0 * p->k);
