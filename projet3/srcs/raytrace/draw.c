...
/*   Updated: 2019/09/17 17:21:00 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/* 
 * brings color values 0 - 1 to  0 255
 *
 */
...
/* Edition mode doesn't require light | render only with object's color */ 
...
void			ft_render(t_scene *s, Uint32 *pixels)
{
	...
	while (++x < W_W)
	{
		...
		while (++y < W_H)
		{
			...
			while (--s->a >= 0)  /* a variable controls Anti-aliasing quality , bigger value == bigger resolution*/
			{
				inter.ray = ft_camera_ray(&s->cam, x + (float)s->a /
						s->nb_samples, y + (float)s->a / s->nb_samples);
				s->c = ft_col3_sum(s->c, s->ft_rtv1(s, &inter, 0));
			}
			pixels[x + (W_H - 1 - y) * W_W] = ft_clamp_gama(s->c.red,   /* interpolate the values for each pixel */
					s->c.green, s->c.blue, 1.0 / s->nb_samples);
		}
	}
//	ft_filter_apply(s->filter, pixels);
}
...
//	ft_filter_apply(g->s.filter, g->pixels);
	return (0);
}
