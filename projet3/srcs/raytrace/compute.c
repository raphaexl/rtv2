...
/*   Updated: 2019/09/17 13:47:45 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_scene_intersectl(t_scene *s, t_intersect *in)
{
	...
	while (p)
	{
		...
		if (p->type == SPHERE && ft_sphere_intersect(&p->e.sphere, &r, &in->t))
			...
			return (1);
		else if (p->type == CYLINDER && ft_cylindre_intersect(&p->e.cylinder, &r, &in->t))
			return (1);
			...
			return (1);
		else if (p->type == ELLIPSOID && ft_ellipsoid_intersect(&p->e.ellipse, &r, &in->t))
			return (1);
		else if (p->type == PARABOLOID && ft_paraboloid_intersect(&p->e.parab, &r, &in->t))
			return (1);
		else if (p->type == MESH)
		{
			for (int i = 0; i < p->e.mesh.size; i++)
